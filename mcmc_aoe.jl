...
function alpha_bm(x, z, μ_pr, invΓ_x, invΓ_obs, y)
    lpz = logpos_bm(z, μ_pr, invΓ_x, invΓ_obs, y)
    lpx = logpos_bm(x, μ_pr, invΓ_x, invΓ_obs, y)
...
function mcmc_bm(μ_pr, Γ_pr, Γ_obs, y, N) 
    ...
    propCovRefl = inv(inv(Γ_pr[3:end,3:end]) + dfx' * invΓ_obs * dfx)
    ...
    for i in 1:N
        ...
        α_atm, logposZ, logposX = alpha_bm(x, z_atm, μ_pr, invΓ_x, invΓ_obs, y)
        if rand() < α_atm
            x[1:2] = z_atm[1:2]
            # logposX = logposZ
            acceptAtm[i] += 1
        ...
        α_ref, logposZ, logposX = alpha_bm(x, z_ref, μ_pr, invΓ_x, invΓ_obs, y)
        if rand() < α_ref
            x[3:end] = z_ref[3:end]
            # logposX = logposZ
            acceptRefl[i] += 1
        ...
        if i % 500 == 0
            if i % 1000 == 0
                ...
                display(cov(x_vals[1:2,1:i], dims=2))
            end
