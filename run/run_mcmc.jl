...
# # LIS for X|Y
# L = cholesky(Γ_x).L
# trilEig = tril(L' * G' * invΓ_ϵ * G * L) + tril(L' * G' * invΓ_ϵ * G * L)' - diagm(diag(L' * G' * invΓ_ϵ * G * L))
...
# # @time samps = mcmc_linear(μ_z, Γ_z, Γ_Δ_hermitian, F, y, 20, "NUTS")
# # @time samps_lowrank = mcmc_linear_lowrank(μ_x, Γ_ϵ, Φ_r, y, Nsamp, "NUTS")
# # samps_full = Φ_r * samps_lowrank[:,:,1]' + Φ_perp * pr_perp_samp + repeat(μ_x, 1, Nsamp)

# # my own implementation
# x0 = μ_x
# @time samps_lowrank_amm = mcmc_amm(x0, Γ_ϵ, Φ_r, Ξ_r, G, y, 100000, μ_x)
...
# plotImgVec(dropdims(mean(samps_full[:,Int(Nsamp/5):end], dims=2), dims=2), hgt, wdth, "MCMC pos mean")
