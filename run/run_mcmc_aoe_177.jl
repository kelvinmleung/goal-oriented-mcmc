...
@time x_possamp = mcmc_bm_3block(μ_x, Γ_x, Γ_ϵ, y, m)
...
# Load from file
# x_possamp = npzread("data_refl/mcmc_results_fullrank/2block/mcmc_simobs_dark.npy")
# mcmcmean = mean(x_possamp[3:end,:], dims=2)

mcmcmean = mean(x_possamp[3:end,Int(m/2):10:end], dims=2)
...
# ## MCMC Chain plots
plot(x_possamp[1,1:10:m], xlabel="Sample number", ylabel="AOD", title="MCMC Chain", label=false)
